...
`, [Cod_Fact, Cod_Hab, EstadiaDias || 1, Total_Pagar]);
		...
		// El Cod_Est = 1 (Finalizada/Cerrada)
		await connection.query(`
UPDATE Reserva 
SET Cod_Est = 1, 
    Pagado_NoPagado = 1, 
...
`, [TipoPago, Cod_Res]);
		...
		// 4. Se ha ELIMINADO la línea que cambiaba el estado de la habitación a Limpieza (Est_Hab = 3).
		// La habitación mantendrá el estado actual (OCUPADO=2) para permitir el cambio manual posterior.
		// await connection.query('UPDATE Habitaciones SET Est_Hab = 3 WHERE Cod_Hab = ?', [Cod_Hab]); 
		
		await connection.commit();
		
		// 5. MENSAJE AJUSTADO: El mensaje ya no dice que la habitación está en limpieza.
